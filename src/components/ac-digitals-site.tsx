    ...
    <main>
      ...
      <section id="quote" className="section-shell bg-secondary/25"><div className="mx-auto max-w-5xl"><div className="section-heading centered"><p className="eyebrow">Project enquiry</p><h2>Request your<br/><span>website quote.</span></h2><p>Tell us about your business and what you're looking for. More context helps us understand your project better.</p></div>
        ...
        <form onSubmit={submit} className="quote-form mt-14" noValidate>
          ...
          <fieldset><legend>Do you already have a website? <b>*</b></legend><div className="choice-row">{[["Yes",true],["No",false]] as const satisfies readonly (readonly [string, boolean])[] as readonly (readonly [string, boolean])[] ? ([["Yes",true],["No",false]] as const).map(([label,value])=><label key={label} className={form.hasWebsite===value ? "active" : ""}><input type="radio" name="hasWebsite" checked={form.hasWebsite===value} onChange={()=>update("hasWebsite",value)}/>{label}</label>) : null}</div></fieldset>
